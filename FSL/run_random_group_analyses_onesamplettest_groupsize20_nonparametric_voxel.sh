	...
	do
		...
		do
			...
			echo "Starting random group analysis $Comparison !"
				...
				./RandomiseGroupLevel all_subjects.nii.gz -groupmean -mask MNI152_T1_2mm_brain_mask.nii.gz -permutations 1000 -inferencemode 0 -permutationfile permutations_onesamplettest_groupsize20.txt -quiet > log.txt

				# Equivalent call to randomise
				#randomise -i all_subjects.nii.gz -o test -1 -P -x -n 100 -m MNI152_T1_2mm_brain_mask.nii.gz
			...
			echo "Out of $Comparison random group analyses, significant group activations were detected $SignificantActivations times !" > /home/andek/Research_projects/RandomGroupAnalyses/Results/results_broccoli_onesamplettest_${Study}_${Design}_${Smoothing}_perm_voxel_groupsize20.txt
