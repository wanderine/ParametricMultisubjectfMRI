	...
	do
		...
		do
			...
			echo "Starting random group comparison $Comparison !"
				...
				./RandomiseGroupLevel all_subjects.nii.gz -design design_matrix_twosamplettest_groupsize10.mat -contrasts contrasts_twosamplettest_groupsize10.con -mask MNI152_T1_2mm_brain_mask.nii.gz -permutations 1000 -quiet -cdt ${ClusterDefiningThreshold} -permutationfile permutations_twosamplettest_groupsize10.txt > log.txt

				# Equivalent call to randomise
				#randomise -i all_subjects.nii.gz -o test -d design_matrix_twosamplettest_groupsize10_fsl.mat -t contrasts_twosamplettest_groupsize10_fsl.con -P -c ${ClusterDefiningThreshold} -n 1001 -m MNI152_T1_2mm_brain_mask.nii.gz
			...
			echo "Out of $Comparison random group comparisons, significant group differences were detected $SignificantDifferences times !" > /home/andek/Research_projects/RandomGroupAnalyses/Results/results_broccoli_twosamplettest_${Study}_${Design}_${Smoothing}_perm_${ClusterDefiningThreshold}_groupsize10.txt
