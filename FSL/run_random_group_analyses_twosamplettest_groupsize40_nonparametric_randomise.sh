	...
	do
		...
		do
			...
			echo "Starting random group comparison $Comparison !"
				...
				randomise -i all_subjects${threads}.nii.gz -o test${threads} -d design_matrix_twosamplettest_groupsize40_fsl.mat -t contrasts_twosamplettest_groupsize40_fsl.con -P -c ${ClusterDefiningThreshold} -n 1000 -m MNI152_T1_2mm_brain_mask.nii.gz --quiet  &
				((threads++))
				...
				if [ "$threads" -eq "$MaximumThreads" ]; then
					...
					echo "Out of $Comparison random group analyses, significant group differences were detected $SignificantDifferences times !"

				fi

			done

			echo "Out of $Comparison random group comparisons, significant group differences were detected $SignificantDifferences times !" > /home/andek/Research_projects/RandomGroupAnalyses/Results/results_randomise_twosamplettest_${Study}_${Design}_${Smoothing}_perm_${ClusterDefiningThreshold}_groupsize40.txt
